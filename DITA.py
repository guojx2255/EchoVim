...
class ILA(nn.Module):
    def __init__(self, T=10, d_model=768, patch_size=16, input_resolution=224, is_training=True):
        super().__init__()
        self.T = T
    ...
    def forward(self, pre, cur):
        ...
        pairs = pairs.reshape(-1, D * 2, T, H, W)   #    8 1536 7 14 14
...
if __name__ == '__main__':
    model = ILA()
    x = torch.randn(2, 9, 196, 768)
    summary(model, input_size=[(2, 9, 196, 768), (2, 9, 196, 768)])

    out, frame, feature = model(x, x)
    print(out.size())
    print(frame.size())
    print(feature.size())