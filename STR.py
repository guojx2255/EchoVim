...
class Reins(nn.Module):
    ...
    def forward(
        ...
        delta_feat = self.forward_delta_feat(     # 计算相似图
            feats,
            tokens,
            layer,
        )
        delta_feat = delta_feat * self.scale
        feats = feats + delta_feat    # 细化后的特征
        if has_cls_token:
    ...
    def forward_delta_feat(self, feats: Tensor, tokens: Tensor, layers: int) -> Tensor:
        attn = torch.einsum("nbc,mc->nbm", feats, tokens)   # 相似图
        if self.use_softmax:
            attn = attn * (self.embed_dims**-0.5)
            attn = F.softmax(attn, dim=-1)   # softmax 相似图
        delta_f = torch.einsum(
            "nbm,mc->nbc",
            attn[:, :, 1:],
            self.mlp_token2feat(tokens[1:, :]),   #初步的细化特征
        )
